...
const handleLogin = async () => {
  if (
    ...
    try {
      const { data, error } = await auth.login(userData.value);

      if (error.value) {
        errorMessage.value = error.value.data;
        console.log("error", errorMessage.value.error);
      }
...
<template>
  <section class="bg-gray-50 dark:bg-gray-900 min-h-screen h-screen">
    <div
    ...
    <toaster v-if="errorMessage" :message="errorMessage" />
  </section>
</template>
<style scoped></style>
